    ...
    if event.reply_to_msg_id is not None:
        ...
        if input_str == "on":
            ...
            if len(lsts) > 0:
                ...
                output_str = "no AI enabled users / chats."
            if len(output_str) > Config.MAX_MESSAGE_SIZE_LIMIT:
                with io.BytesIO(str.encode(output_str)) as out_file:
                    out_file.name = "AI_Bot.text"
                    await event.client.send_file(
                        event.chat_id,
                        out_file,
                        force_document=True,
                        allow_cache=False,
                        caption="AI enabled users",
                        reply_to=event
                    )
            else:
                await event.edit(output_str)
        else:
            await event.edit("Reply To User Message to Add / Delete them from AI.")
    else:
        await event.edit("Reply To A User's Message to Add / Delete them from AI.")
